      ...
      <section className={`flex justify-center ${styles.asked}`}>
        ...
        <div className={` flex gap-10 Container `}>
          {productsData.map((product, index) => (
            <div className={` flex  flex-col w-full ${styles.singleBox}`}>
            ...
            <Image src={product.mainfoto} width={200} height={200} alt={product.name} />
              ...
              <Link href="#" className={styles.anchor} ><h4>{product.name}</h4></Link>
              ...
              <ul className={` ml-8 mt-6 ${styles.listParent}`}>
                    ...
                    {product.description.map((item, index) => (
                      ...
                      <div className={` flex gap-10 justify-center mt-20`}>
                        <Image src={product.online} 
                      width={15}
                      height={15}
                      alt={circle}
                      /> <p>{product.available}</p></div>
<div className="flex items-center justify-center gap-10 mt-10">
<p>{product.price}</p> <Link href="/"><button className="Button_button__6pObN">Zobacz ofertę</button></Link></div>
            </div>
            </div>
          ))}
